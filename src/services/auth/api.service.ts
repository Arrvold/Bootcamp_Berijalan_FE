...
export const tokenInterceptor = async (error: AxiosError): Promise<APIBaseResponse<unknown>> => {
  const cookie = await cookies();

  // 'response' sekarang dikenali tipenya dari AxiosError
  if (error.response?.status === 401) {
    cookie.delete("token");
    redirect("/login", RedirectType.replace);
  }

  // Biarkan errorMessage yang menangani format, kita hanya perlu meneruskan errornya
